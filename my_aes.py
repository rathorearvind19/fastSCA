...
def aes_round(state, roundKey):
    ...
    #if debug:    print('SBOX output: {}'.format(state))
...
def aes_main(state, expandedKey, nbrRounds):
    ...
    #if debug:    print('1st ARK output: {}'.format(state))
    #print(nbrRounds)
    i = 1
    while i < nbrRounds:
        state = aes_round(state,
                               createRoundKey(expandedKey, 16*i))
        #if debug:    print(i, state)
        i += 1
    for k in range(4):
        # iterate over the rows
        for l in range(4):
            state_1[(k*4)+l] = state[(k+(l*4))]
    f = open("temp2_9and10round_Template_RandomKey_RandomPT_Train_1M.txt","a");
    for i in range(len(state_1)):
    	f.writelines(str(state_1[i]) + ' ');
    state = subBytes(state, False)
    state = shiftRows(state, False)
    state = addRoundKey(state,
                             createRoundKey(expandedKey, 16*nbrRounds))
    #if debug:    print(10, state)
    for k in range(4):
        # iterate over the rows
        for l in range(4):
...
def encrypt(iput, key, size):
    #if debug:    print('PT: {}, Key: {}, Size: {}'.format(iput, key, size))
    output = [0] * 16
    last_state_output = [0] * 16
    ...
    #if debug:    print('# of Rounds: {}'.format(nbrRounds))
